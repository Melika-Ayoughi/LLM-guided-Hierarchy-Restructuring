...
for i in "${!hierarchy_names[@]}"; do
    ...
    dim_arg="-dim ${dims[$i]}"
    dset_arg="-dset ./hierarchies/${hierarchy_name}/${hierarchy_name}_optimized/${hierarchy_name}_optimized_adjacency.csv"
    checkpoint_arg="-checkpoint ./hierarchies/${hierarchy_name}/${hierarchy_name}_optimized/${hierarchy_name}_optimized.bin"
